...
func BuildSaveAsMenu(app *App) *fyne.MenuItem {
	return fyne.NewMenuItem("Save As...", func() {
		saveFileDialog(app)
	})
}

func BuildSaveMenu(app *App) *fyne.MenuItem {
...
func BuildNewMenu(app *App) *fyne.MenuItem {
...
func BuildOpenMenu(app *App) *fyne.MenuItem {
	return fyne.NewMenuItem("Open...", func() {
		dialog.ShowFileOpen(func(uri fyne.URIReadCloser, e error) {
			...
			app.State.SetFilePath(uri.URI().Path())
			imgColors := misc.GetImageColors(image)
			i := 0
...
func BuildMenus(app *App) *fyne.Menu {
	return fyne.NewMenu(
		"File",
		BuildNewMenu(app),
		BuildOpenMenu(app),
